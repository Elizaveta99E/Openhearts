...
router.get('/:id', async (req, res) => {
  try {
    const conditions = await Conditions.findByPk(req.params.id);
    if (!conditions) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const newConditions = await Conditions.create({ Name });
...
router.put('/:id', async (req, res) => {
  try {
    const { Name } = req.body;
    const [updated] = await Conditions.update(
      { Name },
    ...
    const updatedConditions = await Conditions.findByPk(req.params.id);
...
router.delete('/:id', async (req, res) => {
  try {
    const deleted = await Conditions.destroy({
      where: { id: req.params.id }
    });
    if (!deleted) {
      return res.status(404).json({ error: 'Условие не найдено' });
